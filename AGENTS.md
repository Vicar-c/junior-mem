...
## MCP Server (knowledge-mcp.cjs)

JSON-RPC over stdin/stdout. Registered via `.mcp.json`.
...
## Report Server (report-server.cjs)

Lightweight HTTP server for reviewing consolidation results and submitting feedback.
...
- **Node.js >= 18** — MCP server and report server
